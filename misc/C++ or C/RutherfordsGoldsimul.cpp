...
//********************LUKE MOORE********************\\
//**************************************************\\
//**SIMULATION OF RUTHERFORDS SCATTERING EXPERIMENT**\\
//***************************************************\\
//***************************************************\\
...
#include <fstream>
using namespace std;
const double t = 0.001; //sets the time interval over which calculations are made as a constant and sets the value
//define function to calculate the radial distance between the alpha particle and the gold particle taken as the origin
...
{
     double y, x, vel_x, vel_y, r, start; //defines double variables x and y coordinates x and y velocities
     //and r radial distance value and start value which is value of r at time = 0

        y = -10.000000000; //initial values of variables
        x = 0.000000000;
        vel_x = 0.0;
        vel_y = 0.0;
        r = 0.0;
        ...
        cout << "Enter a value for the x axis offset:  "; //prompt user for x axis offset value
        cin >> x; //reads in value from user
        cout << "Enter a value for the initial velocity in terms of c: (must be between 0 and 1)   ";
        //prompt user for initial velocity straight up along y axis, vel y.
        cin >> vel_y; // reads in value from user

        start = sqrt((x*x)+(y*y)); //calculates radial distance from origin to particle at t = 0
        std::cout << std::setprecision(10) << "start =  " << start << "\n"; //prints to screen the value of start to 10 Sig Figs

        ofstream myfile;
        myfile.open ("coords.txt"); //opens file and creates it if not already in existence
        ...
        } while (r <= start); //runs do loop until radial distance between origin and
