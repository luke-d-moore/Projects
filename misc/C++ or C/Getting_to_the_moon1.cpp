...
{
    public:
        double rvalue(double Moonx, double Moony) //need to take in the values of the x and y coordinate for this function
{
    return sqrt((Moonx*Moonx)+(Moony*Moony)); // returns the answer by using pythagoras's theorem
}
//define function to calculate the velocity of moon in x direction
double velocityx(double moonearthdist, double Moonx, double Moonvel_x)
// need to take in value of r for use in value of gravitational force,
//x coordinate and the previous velocity in x direction
{
    double F_x, F, acc_x; // define variables for use in equations
        F = G*MoonMass*EarthMass/(moonearthdist*moonearthdist); // defines the gravitational force
        F_x = -F*(Moonx/moonearthdist); // resolves the x component of the gravitational force, F*cos(theta), cos(theta) = adjacent/hyp = x/r
        acc_x = F_x/MoonMass; // F = m * a, newtons equation to determine the acceleration of the moon
        Moonvel_x = Moonvel_x + acc_x* t; // standard constant acceleration equation to determine the final velocity from
        //current velocity and time over which acceleration acts for
    return Moonvel_x; // returns velocity in x direction
}
//define function to calculate the velocity of moon in y direction
double velocityy(double moonearthdist, double Moony, double Moonvel_y)
// need to take in value of r for use in value of gravitational force,
//y coordinate and the previous velocity in y direction
{
    double F_y, F, acc_y;// define variables for use in equations
        F = G*MoonMass*EarthMass/(moonearthdist*moonearthdist);// defines the gravitational force
        F_y = -F*(Moony/moonearthdist);// resolves the y component of the gravitational force, F*sin(theta), sin(theta) = opposite/hyp = y/r
        acc_y = F_y/MoonMass;// F = m * a, newtons equation to determine the acceleration of the particle
        Moonvel_y = Moonvel_y + acc_y* t;// standard constant acceleration equation to determine the final velocity from
        //current velocity and time over which acceleration acts for
    return Moonvel_y; // returns velocity in y direction
}
//define function to calculate the x coordinate of moon
double MoonPos_x(double Moonx, double Moonvel_x) // need to take in velocity in x direction and velocity in x direction
{
        Moonx = Moonx + Moonvel_x * t; // calculates the new x coordinate from old coordinate and velocity acting for particular time
    return Moonx; // returns x coordinate
}
//define function to calculate the y coordinate of moon
double MoonPos_y(double Moony, double Moonvel_y)// need to take in velocity in x direction and velocity in x direction
{
        Moony = Moony + Moonvel_y * t;// calculates the new y coordinate from old coordinate and velocity acting for particular time
...
int main()
{
        ...
        double Moonx = 0.0, Moony = 3.844e8, Moonvel_x = 1018.33, Moonvel_y = 0.0, moonearthdist, newMoonx=0.0, newMoony=0.0;
        double x=0.0, y=Earthradius, vel_x = 0.0, vel_y = 0.0, rmoon = 0.0, r = 0.0, massrocket = 0.0, Angle_adjust = 0.0;
        ...
        cout << endl << "11120 vel_y and roughly (19.2 - 20.2) degree angle adjustment hits the moon"<< endl << endl;
        cout << "Roughly (20.2 - 20.9) slingshots around the moon and comes backwards " << endl << endl;
        ...
        while (i<imax)
        {
            ...
            moonearthdist = TheMoon.rvalue(Moonx,Moony);
            Moonvel_x = TheMoon.velocityx(moonearthdist, Moonx,Moonvel_x);
            Moonvel_y = TheMoon.velocityy(moonearthdist, Moony,Moonvel_y);
            Moonx = TheMoon.MoonPos_x(Moonx, Moonvel_x);
            Moony = TheMoon.MoonPos_y(Moony, Moonvel_y);
